...
import { Frame as CosmicFrame } from "../../cosmic/frame";

const framePaths = JSON.parse(
...
function Frame({ className, frameClassName, style, ...props }: FrameProps) {
	return (
		<div
			{...props}
			style={
				{
					...
					"--color-frame-4-fill":
						"color-mix(in oklab, var(--color-accent) 20%, transparent)",
					"--color-frame-5-stroke":
						"color-mix(in oklab, var(--color-primary) 23%, transparent)",
					"--color-frame-5-fill": "transparent",
					...style,
				} as React.CSSProperties
			}
			className={twMerge(["relative size-90", className])}
		>
			<CosmicFrame
				className={twMerge([
					"drop-shadow-[0_0_28px_var(--color-primary)]",
					frameClassName,
				])}
				paths={framePaths}
			/>
